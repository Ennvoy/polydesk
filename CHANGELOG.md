...
### 2026-08-20｜TUI 滑鼠追蹤下的滾輪捲動

- Claude Code 這類 TUI 會開啟滑鼠追蹤（`?1003`），xterm 據此把滾輪事件當成滑鼠回報送給程式而不捲 scrollback；而這類 TUI 並不使用滾輪，等同滾輪失效。xterm 亦未實作 Shift 逃生口，實測 Shift+滾輪同樣捲 0 行，使用者沒有任何回看歷史的方法。
- 改以畫面模式為判準：主畫面（程式把輸出印在有 scrollback 的緩衝區，如 Claude／Codex）且滑鼠追蹤開啟時，由 Polydesk 捲動終端機自身 scrollback 並不再回報給程式；替代畫面（vim、htop 這類全螢幕接管、本無 scrollback）維持原樣送給程式。滑鼠追蹤未開啟時完全交還 xterm，原生手感不變。
- 捲動量沿用 xterm 的像素模型（deltaY ÷ 實際列高），避免同一終端機在開啟 TUI 前後滾輪速度不一致。實作使用 xterm 公開 API，未觸及私有內部。
- 補真 Electron 回歸測試：以真實滑鼠一格（deltaY 120）驗證修正前捲 0 行、修正後捲 6 行；並保留未開滑鼠追蹤時的基準線斷言。
