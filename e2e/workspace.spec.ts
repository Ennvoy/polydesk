...
test('空白歡迎頁可開啟 Clone Git Repository 對話框', async () => {
  const { app, page, userData } = await launchApp();
  await page.getByRole('button', { name: 'Clone Git Repository' }).click();
  await expect(page.getByRole('dialog')).toContainText('Clone Git Repository');
  await expect(page.getByLabel('Repository URL')).toBeVisible();
  await expect(page.getByLabel('Clone 並開啟 Repository')).toBeDisabled();
  await page.getByLabel('取消 Clone Repository').click();
