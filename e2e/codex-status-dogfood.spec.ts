// 真實 Codex 狀態 smoke test：不送 prompt、不呼叫模型、不消耗額度。
...
test.skip(process.env.POLYDESK_DOGFOOD_CODEX_STATUS !== '1', '需要本機已安裝並登入 Codex');
test.setTimeout(90_000);

test('真實 Codex 啟動停在輸入列顯示已停止，離開後回未啟動', async () => {
  const root = mkdtempSync(join(tmpdir(), 'pd-codex-status-'));
  const dir = join(root, 'codex-status-ws');
  mkdirSync(dir, { recursive: true });
  const { app, page, userData } = await launchApp();
  try {
    await stubFolderPicker(app, [dir]);
    await addWorkspaceViaUI(page);
    await page.locator('button[aria-label="開啟工作區 codex-status-ws"]').click();
    ...
    const stopped = page.getByRole('status', { name: 'Codex 狀態：已停止' });
    await expect(stopped).toBeVisible({ timeout: 35_000 });

    // Codex TUI 的 Ctrl+C 是取消目前操作，不保證退出；關閉終端機才是確定的程序生命週期結束。
