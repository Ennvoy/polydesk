...
test('編輯器隱藏時點檔案：自動顯示編輯器並開檔（含已開分頁再點）', async () => {
  ...
  writeFileSync(join(dir, 'c.md'), 'CCC_REVEAL gamma\n');
  ...
  const tabC = page.locator('[role="tab"][aria-label^="c.md"]');
  ...
  await expect(editorToggle).toHaveAttribute('aria-pressed', 'false', { timeout: 8000 });
  await page.locator('[role="tree"] [role="treeitem"][aria-label="c.md"]').click();
  await expect(editorToggle).toHaveAttribute('aria-pressed', 'true', { timeout: 8000 });
  await expect(tabC).toBeVisible({ timeout: 15000 });
  await expect(page.locator('.monaco-editor').first()).toContainText('CCC_REVEAL', { timeout: 15000 });
