...
test('C ref 徽章：本地 main（HEAD）與遠端 origin/main 各標在所在 commit（like VSCode）', async () => {
  const repo = initRepo('pd-refs-');
...
test('B 切換分支：未提交變更 → 彈窗 Stash 並切換、變更不丟', async () => {
  ...
  // aria-label 斷言，同 2da6306）。30s：斷言的是「最終一致」——剛 build 完磁碟高壓下
  // stash+checkout+refresh 全鏈實測可拖 >15s（狀態列先變 dev、面板後收斂），放寬只影響失敗等待。
  await expect(page.locator('button[aria-label="目前分支 dev"]')).toBeVisible({ timeout: 30000 });
