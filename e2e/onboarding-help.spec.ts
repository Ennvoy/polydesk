...
test('首次導覽會保存進度、續接並在完成後不再自動出現', async () => {
  ...
  try {
    ...
    await expect.poll(() => readOnboarding(userData)).toEqual({ version: 1, status: 'in-progress', step: 2 });
    ...
    await expect.poll(() => readOnboarding(userData)).toEqual({ version: 1, status: 'completed', step: 0 });
...
test('schema v2 與舊導覽版本從第 1 步開始，缺少目標時提供不阻塞替代內容', async () => {
  ...
  const staleUserData = seedState(3, { version: 0, status: 'completed', step: 6 });
  ...
  try {
    ...
    await expect.poll(() => readOnboarding(legacyUserData)).toEqual({ version: 1, status: 'skipped', step: 0 });
    ...
    await stale.page.getByRole('button', { name: '略過導覽' }).click();
...
test('說明與設定共用入口可搜尋完整指南，手動導覽不改寫首次狀態', async () => {
  test.setTimeout(120_000);
  const apps: ElectronApplication[] = [];
  const userData = seedState(3, { version: 1, status: 'completed', step: 0 });
