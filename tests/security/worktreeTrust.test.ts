...
import { verifyWorktreeLineage, canonicalPath } from '../../src/main/git/GitService';

// 假 svc：只需 gitCommonDir(wsId) → 路徑（模擬 git rev-parse --git-common-dir 結果）
function fakeSvc(map: Record<string, string | null>) {
  return { gitCommonDir: async (wsId: string) => map[wsId] ?? null } as unknown as import('../../src/main/git/GitService').GitService;
}
