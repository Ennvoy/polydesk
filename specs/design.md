...
- `git:cleanupExecute` 在同 repository queue 內重驗 lease、建立 prepared journal，再依 checkpoint 執行 worktree、local ref、branch config／reflog，最後才做 remote compare-and-delete 與 tracking ref CAS。遠端租約同時接受原始本機 ref 身分與計畫內刪除 target 後的預期身分，其他變動仍 fail-closed。
- `git:cleanupStatus` 只回去密待辦；`git:cleanupCancel` 限 prepared 零副作用；`git:cleanupResume` 先驗 envelope/payload checksum 與 repository generation，再沿原 checkpoint 繼續。quarantine 或無法重建的 claim 不因封存解鎖；`git:cleanupImportEvidence` 只接受 checksum 完全相符且屬於同 repository generation 的原始 payload。
