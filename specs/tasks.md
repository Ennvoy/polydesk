...
- [x] **DF-14 終端機 AI CLI 快捷啟動**（2026-07-21 dogfood 需求：在終端工具列直接開 Claude／Codex／Agy，Claude 使用 bypass）— 終端機面板新增 `Claude bypass`、`Codex`、`Agy` 三個按鈕；每次點擊沿用目前 shell 建立獨立終端機、自動命名，並在 TerminalView 完成掛載後送出固定命令，避免 CLI 初始輸出遺失。Claude 明確執行 `claude --dangerously-skip-permissions`，以警示色與 tooltip 揭露略過權限確認的風險。驗證：typecheck、build、真 Electron／PowerShell E2E 1 案（隔離假 CLI 驗三命令與 bypass 參數）、既有 terminal-manage E2E 5 案全綠；完整 Vitest 的 Windows 平行資源競爭失敗檔改以單 worker 重跑 12 案全綠。
- [x] **DF-15 AI CLI 快捷啟動先同步 PTY 尺寸**（2026-07-22 dogfood 回報：按 `Claude bypass` 會跑版，手動輸入正常）— 根因是快捷命令只等 TerminalView 掛載就送出，但首次 `fit + pty:resize` 要到下一個動畫幀才完成，Claude 因此先按 ConPTY 預設 `80x24` 畫首屏，再切到面板實際列欄而錯位。修法：TerminalView 首次取得有效 dimensions 後等待 resize IPC 完成，再以一次性 callback 通知 TerminalPanel 送命令；一般手動終端與後續 resize 自癒不變。驗證：typecheck、build、AI launch 真 Electron／PowerShell E2E 1 案，以及 resize-heal／fit-clip／bottom-visibility 5 案全綠。
- [x] **DF-16 終端機檔案路徑 Ctrl+點擊開啟**（2026-07-22 dogfood 需求：Claude 輸出的檔案／截圖路徑希望可直接點開）— xterm LinkProvider 辨識 Windows 絕對路徑、`~\...`、工作區相對路徑、引號內含空白路徑與 `path:line:column`；`Ctrl+左鍵` 由 terminal host capture 依公開 buffer／viewport 座標確定性命中，避免 xterm 6 WebGL／selection 組合下 hover 正常但 `link.activate` 未觸發。工作區內經 main realpath containment 後回 editorBus 開檔跳行欄；工作區外只允許既有一般檔，原生確認後才 `shell.openPath`，並封鎖執行檔、腳本、捷徑、UNC／device prefix、控制字元與 NTFS ADS。驗證：完整 Vitest 59 檔 521 案、typecheck 全綠；terminal-file-link 2 案真 Electron E2E（工作區內開檔定位＋外部確認／腳本拒絕）與 terminal clipboard／SIGINT／OSC52／右鍵 TUI／多終端管理回歸合計 13 案全綠。
- [x] **DF-17 SCM 讀取與分支切換效能優化**（2026-07-22 dogfood 回報：開啟原始碼控制與 marketing-hub 切換分支長時間轉圈）— 根因不是 repo 大，而是 renderer 的 SCM／活動列／狀態列重複查詢、`status` 與 `changes` 各自完整掃描，以及同工作區 Git 序列佇列把數秒級程序逐一累加。新增 `git:snapshot` 讓一次 porcelain 同回狀態與變更；renderer 以 per-workspace single-flight 合併並行查詢；fetch 沿用最新快照、背景探測直接套用結果；branch list 由本地 refs／遠端 refs／current 三程序合併成一次 `for-each-ref`，checkout 後不再重讀未改變的分支清單。不忽略 `.flow` 或 `.obsidian`，保留正確變更偵測。驗證：完整 Vitest 60 檔 525 案、typecheck、build，以及真 Electron SCM 徽章／dirty 與 untracked checkout 共 5 案全綠。
- [x] **DF-18 終端 resize 真實套用確認與失敗重試**（2026-07-22 dogfood 回報：Claude bypass 修正後仍偶發右側裁切）— 根因是 `PtyManager.resize` 捕捉 ConPTY 例外後仍固定回 `{ok:true}`，renderer 因此把 IPC 完成誤認為尺寸已同步；啟動後的版面變更若 resize 失敗一次，在靜止歡迎畫面也沒有輸出觸發後續自癒。修法：`pty:resize` 回傳 `applied` 與實際欄列，快捷啟動只在 xterm／ConPTY 完全一致後送命令，後續 resize 未套用也排程重試。驗證：PtyManager 21 案、typecheck、build、AI launch 真 Electron E2E（假 Claude 回報 PTY 欄數並與 xterm 對賬）全綠。
- [x] **DF-19 AI 快捷啟動尺寸穩定窗**（2026-07-22 dogfood 回報：Claude bypass 首屏偶發跑版；截圖動態輸入區正常、僅靜態歡迎橫幅殘影＝資訊列被截斷且尾巴繞到左欄）— 根因是 DF-15/18 只等「首次 resize 確認」就送命令，但掛載後零點幾秒內版面仍可能再變一次（版面收斂尾巴、字型 fonts.load 後格寬改變、resize 失敗 350ms 補送）；遲到的 resize 撞上 Claude 繪製靜態橫幅即定格殘影（Ink 不重畫靜態區），時序要剛好相撞故偶發。修法：TerminalView 啟動閘門升級為穩定窗——尺寸確認套用後靜置 250ms 無再變動且字型決議完成才通知 TerminalPanel 送命令；新確認重新計時、同尺寸重複確認不重置倒數、倒數到點再核對當下欄列。驗證：typecheck、完整 Vitest、build、AI launch E2E 補穩定窗回歸（命令送出後 600ms 內欄數不變）全綠。
- [x] **DF-20 終端檔案連結格位修正與誤判收斂**（2026-07-23 dogfood 回報：檔案連結判定怪異且無法開啟）— 根因是 parser 的 JavaScript 字串索引被直接拿去當 xterm 格位；中文、全形字、emoji 佔兩格，位於路徑前方時裝飾與 Ctrl+點擊命中便偏移。新增 buffer cell 寬度映射，讓 LinkProvider 與 host capture 共用真實 `cellStart/cellEnd`；同時要求未明示 `./`、`../` 的相對路徑具備副檔名，排除 `N/A`、`workflow/subagent`、`API/資料表` 等假連結。驗證：連結單測 5 案、typecheck、build，以及含中文＋emoji 的開檔定位與外部安全開啟真 Electron E2E 2 案全綠。
- [x] **DF-21 worktree 顯示 AI 執行狀態標籤**（2026-07-23 dogfood 回報：worktree 動作中不顯示 Claude／Codex／Agy 標籤）— 根因是 WorkspaceRail 把 worktree `⎇` 圖示與 `ClaudeStatusBadge` 寫成互斥分支，監控雖已按最長 cwd 路徑正確歸戶，worktree 列卻沒有狀態元件可更新。改為 worktree 類型圖示與 AI 狀態徽章獨立渲染；所有有效工作區皆掛載徽章，missing 工作區維持不掛載。驗證：worktree／狀態歸戶單測 15 案、typecheck、build，以及真 Git／Electron worktree 建立納管與徽章掛載 E2E 1 案全綠。
- [x] **DF-22 編輯器外部改檔同步與分頁批次關閉**（2026-07-23 dogfood 回報：AI 修改已開啟檔案後不會更新，必須關閉再重開；並希望分頁右鍵關閉全部）— 根因是 FileWatcher 遇大量事件會改發 `path=''` 工作區對帳訊號，但 EditorGroup 只處理精準檔案路徑；部分讀檔入口也未保證 watcher 已啟動。新增最後磁碟內容快照與 per-tab async sequence，精準／coarse 事件都只在內容真正不同時更新乾淨文字分頁，dirty 分頁只標記衝突且存檔前仍強制選擇，圖片／Word／試算表預覽同步重載；所有讀取入口先啟動 watcher。分頁右鍵新增關閉／關閉其他／關閉全部，批次限目前工作區、逐一尊重儲存／捨棄／取消，取消立即中止。驗證：typecheck、build、FileWatcher／fileService 42 案，以及新同步、既有衝突與跨工作區分頁真 Electron E2E 4 案全綠。
- [x] **DF-23 終端機 HTTP／HTTPS 網址 Ctrl+點擊外開**（2026-07-24 dogfood 回報：終端輸出的 `http://localhost:3000` 無法點擊）— 根因是 TerminalView 只有檔案路徑 LinkProvider，且檔案解析器明確排除所有 `scheme://`；main 的外開攔截不會主動掃描 xterm buffer，故網址從未進入 `shell.openExternal`。新增純文字網址解析、xterm 實際格位換算及 OSC 8 handler；沿用 `Ctrl+左鍵` 避免干擾選字/TUI，renderer 與 main 共用 HTTP(S) 雙層白名單並透過固定 IPC 外開，拒絕其他協定、控制字元、內嵌帳密與非標準輸入。驗證：網址／安全選項單元測試 12 案、typecheck、build，以及網址外開與既有檔案連結真 Electron E2E 3 案全綠。
- [x] **DF-24 安裝 Sunlike365 後終端機仍可啟動**（2026-07-24 dogfood 回報）— 根因是 PowerShell／CMD 等以裸名稱交給 `node-pty 1.1.0`，其 Windows PATH parser 漏掃沒有尾分號的最後一段；Sunlike365 安裝後重排 PATH 即穩定觸發。Windows 內建 shell 改用 `SystemRoot` 絕對路徑，PowerShell 7／Git Bash 以完整 PATH parser 解析成絕對路徑；`pty:create` 改回結構化錯誤，面板顯示 `shell-not-found`／`no-workspace`／`spawn-failed` 提示。驗證：typecheck、build、PTY／環境單元測試 29 案，以及模擬受影響 PATH 與缺少 shell 提示的真 Electron E2E 2 案全綠。
- [x] **DF-25 Claude Read 工具輸出檔案連結修正**（2026-07-27 dogfood 回報：`Read(C:\...\file.md)` 顯示路徑但 Ctrl+點擊沒有反應）— 根因是既有 matcher 只辨識獨立路徑 token，Claude 的 `Read(` 前綴與 `· lines N-M` 後綴讓整段無法解析。新增工具呼叫格式解析，只連結括號內真正路徑，支援 Windows 絕對／相對路徑、空白與 lines 起始行定位；特殊格式與通用 parser 以文字區間去重，既有 main process containment、外部確認與危險類型封鎖不變。驗證：檔案連結單元測試 7 案、typecheck、build，以及真 Electron hover＋Ctrl 點擊 `Read(... · lines 2-3)` 開檔跳行案例全綠；既有工作區內定位與工作區外安全 E2E 亦通過。
- [x] **DF-26 PATH 重排後保留 AI 執行狀態標籤**（2026-07-27 dogfood 回報：新版終端機已可開啟，但工作區執行標籤消失）— 根因是 DF-24 只把終端機 shell 改為 `SystemRoot` 絕對路徑，AI process scanner 仍以裸名稱執行 WMIC／PowerShell；Windows 11 無 WMIC且 PATH 無法解析 PowerShell 時，Claude／Codex／Agy PID 掃描全數失敗。監控改由不分大小寫的 `SystemRoot`／`windir` 組出兩個系統工具絕對路徑，終端機與監控使用一致的 PATH 相容策略。驗證：程序掃描／狀態監控／版本同步單元測試 17 案、typecheck、diff check、build 與既有 PATH 真 Electron E2E 2 案全綠；回歸案例模擬 PATH 僅含第三方目錄且 WMIC 不可用，PowerShell fallback 仍正確解析三種工具 PID。
- [x] **DF-27 貼上剪貼簿 bitmap 圖片**（2026-07-27 dogfood 回報：同仁無法貼上圖片）— 根因是既有 Explorer Ctrl+V 只支援具有磁碟路徑的外部檔案；截圖工具／瀏覽器／通訊軟體複製的是無路徑 bitmap，`webUtils.getPathForFile()` 必回空字串。新增 `fs:importClipboardImage` 固定 IPC，由 main 直接讀系統剪貼簿並轉 PNG 寫進選取資料夾；預設 `貼上圖片.png`，重名自動 copy、不覆蓋，維持 workspace containment 並限制 20MB。驗證：fileService 32 案、typecheck、diff check、build 全綠；真 Electron E2E 3 案實際寫入 Windows 圖片剪貼簿並按 Ctrl+V，確認落檔、樹更新、圖片預覽與既有檔案貼上皆通過。
- [x] **DF-28 非標準 MIME 虛擬圖片剪貼簿相容**（2026-07-27 二次 dogfood 回報：同仁安裝新軟體的電腦仍無法貼上圖片，懷疑 PATH 異動）— 確認圖片貼上只使用 Electron 系統剪貼簿與 Node 檔案 API，不依賴系統 PATH。真正缺口是部分軟體將圖片公告成 `Files` + 通用／空 MIME 的無路徑虛擬 File，v0.16.0 因沒有 `image/*` 而不會啟動 bitmap fallback。修正為無可用磁碟路徑的 Files 也交由 main 嘗試 `clipboard.readImage()`；實體檔案與純文字貼上不變。驗證：typecheck、diff check、build 全綠；真 Electron Explorer E2E 4 案全綠，新案重現 `application/octet-stream` 無路徑虛擬圖片並確認成功落檔。
- [x] **DF-29 終端內容導覽、四工作區 AI 串流與 SCM 效能**（2026-07-31 dogfood 需求：像 Codex 一樣可依每句輸出跳轉，四個工作區同時跑 Claude／Codex 容易卡，嚴重卡點常在原始碼控制；commit `1d04569`）— 終端左側新增非空邏輯行導覽節點、預覽、目前位置與 `Alt+↑`／`Alt+↓` 鍵盤跳轉；renderer 改用單一 PTY dispatcher，不可見終端釋放 WebGL，main 以 16ms／100ms 處理前景與背景串流，鍵盤輸入後 250ms 內改走 4ms 互動優先，回到前景立即補送。SCM 的狀態列／活動列／面板共用 600ms 快照，檔案事件明確失效；歷史與分支不再因一般 changes 更新重讀，大量變更每批渲染 200 項，背景探測改 10 秒；線圖在 fetch 後同時顯示本地與遠端獨有 commit。驗證：相關單元測試、typecheck、build、真 Electron 導覽與 AI launch E2E 全綠；四工作區高速串流皆超過 50 行後量得 frame p95 19.7ms、renderer CPU 3.6%，通過 50ms／25% budget；20 鍵三輪 p95 22／21／21ms；600 變更 SCM 首批 200 列、四波 snapshot 最多 4 次且額外 git log 為 0。
...
- [x] **DF-34 啟動畫面立即顯示**（2026-08-11 dogfood 回饋：雙擊後應先看到啟動畫面，再等待主程式開啟）— portable 封裝加入 420×230、24-bit RGB 原生 splash，在自解壓階段先顯示既有 Polydesk 畫面；Electron 接手後移除固定 250 ms 顯示門檻，原生視窗建立後立即顯示並在 `show` 事件後才開始主初始化。不設定最低停留時間，主視窗仍等待工作區載入、`ready-to-show` 與 renderer-ready 握手；新增 BMP 封裝契約、首次顯示埋點與真 Electron 回歸，保留 sandbox、外部開窗封鎖、失敗重試與退出行為，首次導覽與完整使用指南內容確認不受影響。
- [x] **DF-35 portable 只保留單一動畫啟動畫面**（2026-08-11 dogfood 回饋：自解壓靜態 BMP 不會轉動，關閉後再跳 Electron 動畫 splash 形成兩次啟動畫面）— 依使用者選擇移除 electron-builder `portable.splashImage` 與 BMP 資產，接受自解壓期間短暫無畫面的取捨；自解壓完成後只建立既有 Electron 動畫 splash，主視窗交接、安全與失敗處理不變。封裝契約測試禁止重新引入靜態 splash；完整 shard 暴露既有真 Git／Windows clipboard 案在負載下的測試競爭，改為先等產品 UI、驗證 clipboard 前置狀態並保留既有產品斷言。另修正 checkout 成功後從 stale status ref 讀回舊分支，以及工作區初始化覆蓋使用者剛選 SCM 分頁的 UI race。最終 ship runner：Vitest 572/572、112 個非豁免 Electron E2E 全綠、3 個真 AI dogfood 跳過；首次導覽與完整使用指南確認不受影響。
