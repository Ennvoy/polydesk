...
| **終端機多開** | 同一工作區可並排/上下多開終端機、可拖曳調整，支援 PowerShell 等 shell；工具列可一鍵建立並啟動 Claude bypass、Codex 或 Agy；選取文字後可用 `Ctrl+C` 在終端機間複製貼上，未選取時仍送出中斷訊號。 |
...
終端機面板右上方提供三個 AI CLI 快捷按鈕：`Claude bypass` 會執行 `claude --dangerously-skip-permissions`，`Codex` 會執行 `codex`，`Agy` 會執行 `agy`。每次點擊都會建立一個獨立終端機，並沿用目前工作區選定的 shell。Claude bypass 會略過所有工具與檔案操作的權限確認，只能在你完全信任的工作區使用。
