...
Polydesk 是以 Electron 打造的桌面應用，專為「同時開多個專案、每個專案各自跑 AI CLI（Claude Code / Codex）」的工作流設計。一個視窗管理所有工作區的終端機、程式碼、版本控制與 AI 執行狀態。
...
| **終端機多開** | 同一工作區可並排/上下多開終端機、可拖曳調整，支援 PowerShell 等 shell。 |
...
| **AI 狀態監控** | 以真實 process 偵測，顯示各工作區 Claude / Codex「執行中 / 待確認 / 已停止」徽章。 |
| **總覽面板** | 一鍵最大化總覽：各工作區 AI 狀態 + 服務用量（5 小時 / 每週額度）。 |
...
```powershell
npm run dist         # 打包成 Portable exe → release/Polydesk-<version>-portable.exe
npm run pack:dir     # 只產生未壓縮的 app 目錄（除錯用）
```
