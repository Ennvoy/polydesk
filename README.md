...
| **終端機多開** | 同一工作區可並排/上下多開終端機、可拖曳調整，支援 PowerShell 等 shell；工具列可一鍵建立並啟動 Claude bypass、Codex 或 Agy，且會先同步實際終端尺寸再啟動 TUI，避免首屏跑版；選取文字後可用 `Ctrl+C` 在終端機間複製貼上，未選取時仍送出中斷訊號。 |
