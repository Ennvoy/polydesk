...
export interface PanelLike {
  ...
  readonly api: {
    ...
    maximize(): void;
    exitMaximized(): void;
    /** panel 所屬 group（顯隱以 group 為單位；group.api.setVisible 隱藏但不 dispose component）。 */
    readonly group: {
      readonly api: {
 ...
 * 保住終端機 PTY/畫面、編輯器開檔/未存編輯）。dockview 的 group setVisible 經
 * splitview.setViewVisible 真正回收 grid 空間（hidden viewItem size→0、minimumSize→0 繞過最小尺寸夾擠、
 * distributeEmptySpace 把空間讓給其他 panel），非「只設旗標不視覺隱藏」。不存在（如舊持久化無此 panel）
 * →以 add 重建。回傳切換後是否「可見」。
 */
