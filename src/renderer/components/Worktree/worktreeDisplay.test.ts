...
describe('靜態守衛：worktree/rail 渲染路徑禁 dangerouslySetInnerHTML（紅軍 A1）', () => {
  ...
  it('worktree 圖示不會取代 Claude/Codex/Agy 狀態徽章', () => {
    const rail = readFileSync(join(__dirname, '..', 'WorkspaceRail.tsx'), 'utf8');
    expect(rail).toContain("{!isMissing && <ClaudeStatusBadge wsId={w.id} />}");
    expect(rail).not.toMatch(/isWorktree\s*\?[\s\S]{0,300}:\s*\(\s*<ClaudeStatusBadge/);
  });
});
